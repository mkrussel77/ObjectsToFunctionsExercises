...
import com.functional.chapter11.Outcome.Companion.transform2
...
fun <E : OutcomeError, A, B, C, R> ((A, B, C) -> R).transformAndCurry(
    other: Outcome<E, A>
): Outcome<E, (B) -> (C) -> R> = other.transform { curry()(it) }

fun <E : OutcomeError, A, B, C, D, R> ((A, B, C, D) -> R).transformAndCurry(
    other: Outcome<E, A>
): Outcome<E, (B) -> (C) -> (D) -> R> = other.transform { curry()(it) }
...
infix fun <E : OutcomeError, A, B, C, D, R> (
    (A, B, C, D) -> R).`!`(
    other: Outcome<E, A>
): Outcome<E, (B) -> (C) -> (D) -> R> = transformAndCurry(other)
